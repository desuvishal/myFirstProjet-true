#!/usr/bin/env bash
# Local test runner for "Build Your Own BitTorrent Client" (shipthatcode.com).
# Usage:
...
run_one() { # $1 = input file
  ...
  case "$LANG_SLUG" in
    c|cpp|rust|assembly|basic|cobol|d|fortran|pascal) ./.prog decode "$value" ;;
...
pass=0; fail=0
for dir in "$TEST_ROOT"/${FILTER}*/; do
  [ -d "$dir" ] || continue
  for input in "$dir"*.in; do
    [ -f "$input" ] || continue
    expected="${input%.in}.out"
    if ! actual="$(run_one "$input")"; then
      echo "SKIP  $input (unsupported input protocol for the current stage)"
      continue
    fi
